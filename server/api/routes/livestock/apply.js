...
// POST route without image upload
router.post('/', (req, res) => {
  const { name, cnic, mobile_no, address, remarks } = req.body;
  ...
  const sql = 'INSERT INTO applications (name, cnic, mobile_no, address, remarks, created_at) VALUES (?, ?, ?, ?, ?, ?)';
  db.query(sql, [name, cnic, mobile_no, address, remarks, created_at], (err, result) => {
    if (err) {
      console.error(err);
...
router.delete('/:id', checkAuth, (req, res) => {
  ...
  const sql = 'DELETE FROM applications WHERE apply_id = ?'; // Assuming 'id' is the primary key in your applications table
