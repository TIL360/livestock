...
router.get("/", checkAuth, async (req, res) => {
  const { task_standard, created_task_at, user } = req.query; // Extract query parameters
  ...
  if (created_task_at) {
    query += ' AND created_task_at = ?'; // Ensure you are using the correct column name
...
router.get("/:syllabus_id", checkAuth, (req, res) => {
  ...
  db.query("SELECT * FROM syllabus WHERE syllabus_id = ?", [taskId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: "Database query failed" });
    }
...
router.patch("/:syllabus_id", checkAuth, (req, res) => {
  const taskId = req.params.syllabus_id;
  const { standard, subject, task } = req.body;

  // Update the task in the database
  const query = `
      UPDATE syllabus 
      SET task_standard = ?, subject = ?, task = ? 
      WHERE syllabus_id = ?`;

  db.query(query, [standard, subject, task, taskId], (error, result) => {
      if (error) {
